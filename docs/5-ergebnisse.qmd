...
rule_df <- data.frame(
  ...
  `Share of in-matrix (%)` = c("60.8", "35.1", "4.1", "**100.0**", "*—*"),
  check.names = FALSE
)

# Plain pandoc-native markdown table: converts cleanly to both HTML and typst
# output without depending on kableExtra's format detection (which does not
# recognise the custom zhaw-lsfm-typst format and falls back to producing an
# HTML table, which is what triggered the "Functions that produce HTML output
# found in document targeting typst output" error).
...
leaflet(options = leafletOptions(minZoom = 8)) |>
  ...
  hideGroup(c("Dist Secondary Road — Aargau",
              "Dist Fenced Road — Aargau [excluded]",
              "Dist Main Road — Zürich",
              "Dist Secondary Road — Zürich",
              "Dist Railway — Zürich",
...
Of the fourteen spatial covariates assembled for the resistance surface pipeline, the two maps below show its final products: the habitat suitability surface recovered from the fitted iSSF coefficients, and the resistance surface obtained from it through the Keeley transformation, for both Canton Aargau and Canton Zürich.
...
# Copy into the project-local images/ folder so the typst compiler can read it
# (typst refuses to load files outside its project root, i.e. outside docs/).
local_png <- file.path("images", "suitability_resistance_maps_Aargau.png")
file.copy(file.path(data_proc, "suitability_resistance_maps_Aargau.png"), local_png, overwrite = TRUE)
...
file.copy(file.path(data_proc, "suitability_resistance_maps_Zurich.png"), local_png, overwrite = TRUE)
knitr::include_graphics(local_png)
```
:::
...
![Baseline scenario produced by the Wildboar Connectivity (ASF) plugin for a single hypothetical outbreak origin (red point) in Canton Zürich, run without any fences or overpasses applied. Top left: Outbreak origin shown on the Swissimage basemap. Top right: Least-cost path corridors connecting the origin to the habitat destinations (green points), classed by cumulative edge strength into near-origin backbone, intermediate, and distant segments. Centre left: Infection-risk surface classified into the three EFSA-anchored zones (low, medium, high risk). Centre right: Circuit-theory dispersal corridor probability on a Magma colour ramp, concentrating current density into a small number of bright branches that mark the structurally most important crossings around the origin. Bottom left: Biased correlated random walk visitation density for n = 2,000 realisations, forming a compact, well-defined core directly around the origin that fades toward the edge of the analysis window. Bottom right: iSSF-IBMM contamination probability surface (log10-transformed) for n = 2,000 agents, showing a high-probability zone centred on the origin surrounded by a broader, lower-probability halo across the wider landscape.](images/baseline.png){#fig-plugin-baseline}
...
![Comparison of the baseline scenario (left column) with a modified scenario in which four fences and one wildlife overpass were placed on impassable barriers and existing wildlife overpasses near the outbreak origin (shown as red lines and blue points). From top to bottom, the rows show the least-cost path corridors, the circuit-theory dispersal corridor probability, the infection-risk zones, the biased correlated random walk visitation density (n = 2,000), and the iSSF-IBMM contamination probability surface (n = 2,000 agents). Adding the fences visibly reshapes every downstream output along the affected edge: the corridor network and the medium-risk zone are truncated to the south and southwest, the circuit-theory current is rerouted into a narrower channel north of the origin and its peak probability rises from 0.00409 to 0.005633, and the random walk density is compressed into a smaller permeable area with its peak visitation count rising from 291 to 333. Because the same number of walkers and agents is now confined to less of the landscape, the comparison illustrates how a single barrier placement concentrates rather than simply blocks movement, and how the overpass alone is building a new corridor that crosses the highway.](images/with_fence.png){#fig-comparison}

The scenario comparison illustrates the core purpose of the plugin. It enables iterative exploration of management alternatives within the same GIS session. A wildlife manager can test different fence alignments, observe how each one redirects the modelled corridor network, and compare the resulting wild boar corridors and infection-risk zones, all without leaving the QGIS environment or re-running any preprocessing steps.
