...
leaflet(options = leafletOptions(minZoom = 8)) |>
  ...
  # Default view: only Resistance — Zürich (and its legend) shown. Hiding the
  # other three AFTER the legends removes their legends via the groupremove event.
...
# Dedicated palette for distance-to-forest-edge (metres) so its own legend is
# meaningful (this layer was previously drawn on the 0–1 forest-density scale).
...
leaflet(options = leafletOptions(minZoom = 8)) |>
  ...
  # Hide every layer by default (placed AFTER the legends so the groupremove
  # event also removes each hidden layer's legend on load — nothing shown until
  # the user toggles a layer on).
