...
The final conditional logistic regression was fitted on 7,824 in-matrix transit steps from 14 GPS-collared individuals in Canton Aargau, each compared against ten random control steps. All covariates are z-standardised with the Aargau training mean and standard deviation. The seven retained coefficients, ordered by magnitude, are summarised here for reference: distance to railway ($\hat{\beta} = 0.558$), forest density at the 50-metre scale ($\hat{\beta} = 0.501$), forest density gradient at the 1000-metre scale ($\hat{\beta} = 0.260$), distance to main road ($\hat{\beta} = 0.232$), distance to settlement ($\hat{\beta} = 0.226$), distance to secondary road ($\hat{\beta} = 0.169$), and distance to water ($\hat{\beta} = 0.052$). All coefficients were positive and significant at the 0.05 level, and all variance inflation factors for the retained set were below 1.4.
