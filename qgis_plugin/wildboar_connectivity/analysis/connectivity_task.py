...
from ..utils.layer_utils import ZOrder, add_wildboar_layer
...
# Updated with final iSSF coefficients (log_sl = 0.16877, cos_ta = 0.57314)
# =================================================================
IBMM_SL_GAMMA_SHAPE    = 2.66877  # tentative (2.5) + beta_log_sl
IBMM_SL_GAMMA_SCALE_M  = 160.0    # step-length scale (metres)
IBMM_KAPPA             = 1.07314  # tentative (0.5) + beta_cos_ta 
...
class AsfConnectivityTask(QgsTask):
    ...
    def _build_lcps_to_forests(self, arr, win_tf, mcp, cost_grid,
        ...
        paths = []   # list of cell lists
        for _entry_cost, r, c, _size in clusters:
            try:
        ...
        # --- Per-edge strength based on local cost -------------------
        # Strength = max_cost - local_cost, so segments near the origin
        # (low Dijkstra cost) are strong and segments far away decay
        # naturally. Where multiple paths share an edge the strengths
        # accumulate, reinforcing the backbone corridor.
        finite_costs = cost_grid[np.isfinite(cost_grid)]
        max_cost = float(finite_costs.max()) if finite_costs.size > 0 else 1.0
        ...
        for cells in paths:
            for a, b in zip(cells, cells[1:]):
                ...
                ca = float(cost_grid[a[0], a[1]])
                cb = float(cost_grid[b[0], b[1]])
                if not (np.isfinite(ca) and np.isfinite(cb)):
                    continue
                local_cost = (ca + cb) * 0.5
                edge_strength[edge] += max_cost - local_cost
    ...
    def _add_lcp_lines_layer(self):
        ...
        layer = QgsVectorLayer(
            ...
            f"clusters, weighted by cost distance; {len(polys)} segments)",
            "memory")
        prov = layer.dataProvider()
        prov.addAttributes([QgsField("traffic", QVariant.Double)])
        layer.updateFields()

        for p in polys:
            f = QgsFeature()
            f.setGeometry(QgsGeometry.fromPolylineXY(p["points"]))
            f.setAttributes([float(p["traffic"])])
            prov.addFeature(f)
        layer.updateExtents()
        ...
        if max_t <= 0:
            low_hi, med_hi = max_t * 0.33, max_t * 0.66
        else:
            q33 = traffics[len(traffics) // 3]
            q66 = traffics[(2 * len(traffics)) // 3]
            low_hi = max(max_t * 0.01, q33)
            med_hi = max(low_hi * 1.01, q66)
